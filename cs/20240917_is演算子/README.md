...
# is 演算子
if 文のステートメントで使用する場合
- 定数値との比較（`false`、`null` など）
- 型の互換性チェック

`==` 演算子

以降では `is` 演算子に関する言語仕様をまとめます。
...
### var パターン
式の結果を受けるときに型推論を利用して `var` による宣言パターンを書くこともできます。この場合、式の結果がどんな型であるかはあらかじめ分かっているため、`if` 文のステートメントで用いるより、その後の処理で使用する計算の中間結果を保持する一時変数として用いる用途に向いています。
``` cs
